...
import { sendDoctorOtp, verifyDoctorOtp } from "@/lib/auth";
import { INDIAN_PHONE_PREFIX, isValidIndianPhoneInput, sanitizeIndianPhoneInput, toIndianPhoneNumber } from "@/lib/phone";
import { motion } from "framer-motion";
import { ArrowRight, CheckCircle2, Lock, MapPin, Phone, ShieldCheck, Stethoscope, User } from "lucide-react";
import LocationPicker from "@/components/LocationPicker";

export default function DoctorSignup() {
  const navigate = useNavigate();
  const { registerDoctor, setShowSplash } = useAppContext();
  ...
  const handleSendOtp = async () => {
    ...
    const result = await sendDoctorOtp(toIndianPhoneNumber(form.phone));
  ...
  const handleVerifyOtp = async () => {
    ...
    const result = await verifyDoctorOtp(toIndianPhoneNumber(form.phone), otp);
    ...
    setStatusMessage(result.message ?? "OTP verified. You can create your doctor account now.");
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute bottom-[-20%] left-[-10%] h-[520px] w-[520px] rounded-full bg-secondary/5 blur-3xl" />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative glass-card w-full max-w-md p-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground">Doctor Sign Up</h2>
            <p className="text-sm text-muted-foreground">Create your account with phone number and password</p>
          </div>
        </div>
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSignup}>
          ...
          <div className="rounded-xl border border-border bg-muted/30 p-4 space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Verify your Indian mobile number with OTP before doctor signup.</p>
              </div>
            ...
            {otpSent ? (
              <div className="space-y-3">
                ...
                <motion.button
                  ...
                  className="w-full rounded-lg border border-secondary/30 bg-secondary/10 px-4 py-3 text-sm font-semibold text-secondary transition-colors hover:bg-secondary/15 disabled:opacity-70"
                >
          ...
          <div className="space-y-1">
            <p className="text-xs font-medium text-muted-foreground pl-1">Clinic / Practice Location <span className="text-muted-foreground/60">(optional — helps patients find you)</span></p>
            <button
              type="button"
              onClick={() => setShowLocationMap((v) => !v)}
              className="flex items-center gap-2 w-full pl-3 pr-4 py-3 rounded-lg bg-muted/50 border border-border text-left text-foreground hover:bg-muted/70 focus:outline-none focus:ring-2 focus:ring-secondary/50 transition-colors"
            >
              <MapPin className="h-4 w-4 text-muted-foreground shrink-0" />
              <span className={`flex-1 text-sm truncate ${location ? "" : "text-muted-foreground"}`}>
                {location || "Pick your clinic location on map"}
              </span>
              <span className="text-xs text-secondary font-medium">{showLocationMap ? "Hide map" : "Open map"}</span>
            </button>
            {showLocationMap && (
              <LocationPicker
                value={location}
          ...
          <motion.button type="submit" whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} disabled={loading}
            ...
            {loading ? "Creating account..." : "Create Doctor Account"} <ArrowRight className="h-4 w-4" />
          </motion.button>
        </form>

        <p className="mt-5 text-center text-sm text-muted-foreground">
          Already have an account?{" "}
          <Link to="/login/doctor" className="font-semibold text-secondary hover:underline">
            Login here
          </Link>
        </p>
